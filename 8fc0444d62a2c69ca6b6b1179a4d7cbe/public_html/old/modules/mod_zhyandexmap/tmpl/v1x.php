...
$scripttext .= '<script type="text/javascript" >/*<![CDATA[*/' ."\n";
	...
	if (isset($map->toolbar) && (int)$map->toolbar == 1) 
	{
		...
		if (isset($map->autopositioncontrol) && (int)$map->autopositioncontrol == 1) 
		{
				...
				{
					case 1:
						$scripttext .= 'var btnGeoPosition = new YMaps.ToolBarButton({ icon: "'.$imgpathUtils.'geolocation.png", caption: "", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'"});' ."\n";
					break;
					case 2:
						$scripttext .= 'var btnGeoPosition = new YMaps.ToolBarButton({ caption: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'"});' ."\n";
					break;
					case 3:
						$scripttext .= 'var btnGeoPosition = new YMaps.ToolBarButton({ icon: "'.$imgpathUtils.'geolocation.png", caption: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'"});' ."\n";
					break;
					default:
						$scripttext .= 'var btnGeoPosition = new YMaps.ToolBarButton({ icon: "'.$imgpathUtils.'geolocation.png", caption: "", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_AUTOPOSITIONBUTTON').'"});' ."\n";
					break;
		...
		if (isset($map->markerlistpos) && (int)$map->markerlistpos != 0) 
		{
			...
			else
			{
				...
				else
				{
						...
						{
							case 1:
								$scripttext .= 'var btnPlacemarkList = new YMaps.ToolBarToggleButton({ icon: "'.$imgpathUtils.'star.png", caption: "", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_PLACEMARKLIST').'"}'.$btnPlacemarkListOptions.');' ."\n";
							break;
							case 2:
								$scripttext .= 'var btnPlacemarkList = new YMaps.ToolBarToggleButton({ caption: "'.JText::_('MOD_ZHYANDEXMAP_MAP_PLACEMARKLIST').'", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_PLACEMARKLIST').'"}'.$btnPlacemarkListOptions.');' ."\n";
							break;
							case 11:
								$scripttext .= 'var btnPlacemarkList = new YMaps.ToolBarToggleButton({ icon: "'.$imgpathUtils.'star.png", caption: "", hint: "'.JText::_('MOD_ZHYANDEXMAP_MAP_PLACEMARKLIST').'"}'.$btnPlacemarkListOptions.');' ."\n";
							break;
	...
	if (isset($map->markergroupcontrol) && (int)$map->markergroupcontrol != 0) 
	{
   	   if (isset($markergroups) && !empty($markergroups)) 
	   {
		...
		foreach ($markergroups as $key => $currentmarkergroup) 
		{
			...
			if ((int)$currentmarkergroup->overridemarkericon != 0) 
			{
	...
	if ($allowUserMarker == 1)
	{
		...
		$scripttext .= 'insertPlacemark.name = "'.JText::_( 'MOD_ZHYANDEXMAP_MAP_USER_NEWMARKER' ).'";' ."\n";
		$scripttext .= 'insertPlacemark.description = "'.JText::_( 'MOD_ZHYANDEXMAP_MAP_USER_NEWMARKER_DESC' ).'";' ."\n";
	...
	if (isset($markers) && !empty($markers)) 
	{
		...
		foreach ($markers as $key => $currentmarker) 
		{
				   ...
				   )
				{
					...
					if (isset($map->markergroupcontrol) && ((int)$map->markergroupcontrol != 0)
						&& ((int)$currentmarker->markergroup != 0) && ((int)$currentmarker->overridemarkericon == 1) )
					{
	...
	if (isset($routers) && !empty($routers)) 
	{
		...
		foreach ($routers as $key => $currentrouter) 
		{
			...
			if ($currentrouter->route != "")
			{
				...
				$scripttext .=       '   alert(\''.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_GEOCODING').'\' + number);'."\n";
				$scripttext .=       '}); '."\n";
				$scripttext .=       'YMaps.Events.observe('.$routername.', '.$routername.'.Events.RouteError, function (number) {'."\n";
				$scripttext .=       '   alert(\''.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_UNABLE_DRAW_ROUTE').'\' + number);'."\n";
				$scripttext .=       '}); '."\n";
			...
			if ($currentrouter->kmllayerymapsml != "")
			{
				...
				$scripttext .= '    alert(\''.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_YMAPSML').'\' + error);' ."\n";
				$scripttext .= '});' ."\n";
			...
			if ($currentrouter->kmllayerkml != "")
			{
                ...
                $scripttext .= '	alert(\''.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_KML').'\' + error);' ."\n";
				$scripttext .= '});' ."\n";
			}
			...
			if ($currentrouter->kmllayergpx != "")
			{
				...
				$scripttext .= '    alert(\''.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_GPX').'\' + error);' ."\n";
				$scripttext .= '});' ."\n";
			}
...
// Find option Begin
	if (isset($map->findcontrol) && (int)$map->findcontrol == 1) 
	{
        ...
        $scripttext .= '        alert("'.JText::_('MOD_ZHYANDEXMAP_MAP_ERROR_FIND_GEOCODING_ERROR').'" + error);' ."\n";
        $scripttext .= '    })' ."\n";
        $scripttext .= '};' ."\n";
	}
// Find option End
...
// Add Map Route
		if (isset($map->findroute) && (int)$map->findroute == 1) 
		{
			...
			$scripttext .= '       		geoRoute.getWayPoint(0).setIconContent(\''.JText::_('MOD_ZHYANDEXMAP_MAP_FIND_GEOCODING_START_POINT').'\');'."\n";
			
			$scripttext .= '       		geoRoute.getWayPoint(1).setIconContent(\''.JText::_('MOD_ZHYANDEXMAP_MAP_FIND_GEOCODING_END_POINT').'\');'."\n";
			$scripttext .= '       		geoRoute.getWayPoint(1).setBalloonContent(\''.JText::_('MOD_ZHYANDEXMAP_MAP_FIND_GEOCODING_END_POINT').'\');'."\n";
			$scripttext .= '       });'."\n";	
