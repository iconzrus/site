...
{
	public 			$params 	=	 array();
	private static 	$ui_version	=	'1.10.3';
	...
	public function importFiles()
	{
        ...
        $ui_files['uitheme'] = array();

        jimport('joomla.filesystem.folder');

        $ui_files['uitheme'] = array_merge(JFolder::files(
            dirname(__FILE__) . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . $this->params['uitheme'],
            '\.css$'
        ));

        foreach($ui_files['uitheme'] as $key=>$uitheme_file)
        {
            $ui_files['uitheme'][$key] = 'css/' . $this->params['uitheme'] . '/' . $uitheme_file;
        }
