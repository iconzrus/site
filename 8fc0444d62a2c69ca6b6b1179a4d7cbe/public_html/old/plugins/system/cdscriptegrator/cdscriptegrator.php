...
class PlgSystemCDScriptegrator extends JPlugin
{
    ...
    public function onAfterRender()
    {
        ...
        // jQuery UI global style ordering
        if (JString::strpos($buffer, $this->_name . '/libraries/jqueryui') !== false and JString::strpos($buffer, 'css/jquery-ui-core-') !== false)
        {
            preg_match('#<link.*?href=".*?' . $this->_name . '/libraries/jqueryui/(?:css|gzip/css\.php\?files\[\]=css)/jquery-ui-core-.*?\.custom\.css".*?/>#i', $buffer, $jQueryUICoreStyle);

            if (isset($jQueryUICoreStyle[0]))
            {

                $buffer = str_ireplace($jQueryUICoreStyle[0], '', $buffer);
                JResponse::setBody($buffer);

                preg_match_all('#<link.*?href=".*?' . $this->_name . '/libraries/jqueryui/(?:css|gzip/css\.php\?files\[\]=css)/.*?/jquery-ui-.*?\.custom\.css".*?/>#i', $buffer, $jQueryUILastThemeStyles);

                if (isset($jQueryUILastThemeStyles) and is_array($jQueryUILastThemeStyles))
                {
                    $search_last_uitheme = $jQueryUILastThemeStyles[0][count($jQueryUILastThemeStyles[0]) - 1];
                    $replace_last_theme = $jQueryUILastThemeStyles[0][count($jQueryUILastThemeStyles[0]) - 1] . "\n" . $jQueryUICoreStyle[0];

                    $buffer = str_ireplace($search_last_uitheme, $replace_last_theme, $buffer);
                    JResponse::setBody($buffer);
                }
            }
        }
