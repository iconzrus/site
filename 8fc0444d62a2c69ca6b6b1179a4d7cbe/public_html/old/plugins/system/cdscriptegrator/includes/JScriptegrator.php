...
class JScriptegrator
{
                ...
                if (is_callable(array($libraryInstance, 'importFiles')))
                {
                    ...
                    if (is_array($importFiles))
                    {
                        foreach($importFiles as $key=>$importFile)
                        {
                            ...
                            {
                                $files = array_merge($files, array_values($importFile));
                            }
                            else
                            {
                                $files[] = $importFile;
                ...
                if ((int) self::$plugin_params->get('compression', 0))
                {
                    ...
                    foreach($files as $file)
                    {
                        ...
                        switch ($extension)
                        {
                            case 'js':
                                ...
                                break;
                            case 'css':
                                if(strpos($file, 'css/jquery-ui-core-') !== false)
                                {
                                    $global_css_file = $file;
                                    continue;
                                }
                                $css_files []= $file;
                                break;
                    ...
                    if (!is_array($css_files))
                    {
                        ...
                        if (JFile::exists($loader_filepath))
                        {
                            ...
                            // append global jQuery UI CSS file
                            if ($global_css_file)
                            {
                                $document->addStyleSheet($this->folderPath() . "/libraries/$library/gzip/css.php?files[]=" . $global_css_file);
                            }
                        }
                    }
     ...
     * @return 	array
     */
    public function sanitizeRequest($data = array(), $method = 'post', $separator = '_')
    {
